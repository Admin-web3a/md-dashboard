...
def build_lead_record(lead):
    ...
    return {
        ...
        "v": get_custom_field(lead, MD_AB_VARIANT_FIELD_ID) or "",     # A/B variant
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .src-btn:hover {{ border-color: #4caf50; color: var(--text); }}
  .src-btn.active {{ border-color: #4caf50; color: #4caf50; background: #1a2e1a; }}
  ...
  .plan-table tr.section-header td {{
    background: #1f1f2e; color: var(--accent); font-weight: 600;
...
const C = {{
  purple: 'rgba(108,99,255,0.85)', green:  'rgba(76,175,80,0.85)',
  blue:   'rgba(33,150,243,0.85)', orange: 'rgba(255,152,0,0.85)',
  pink:   'rgba(233,30,99,0.85)',  teal:   'rgba(0,188,212,0.85)',
}};
...
function render(leads) {{
  ...
  // Daily leads chart — все дни в пределах текущего фильтра, нулевые включительно
  const dayMap = {{}};
  ...
  leads.filter(l => l.s >= 14).forEach(l => {{
    const ts = l.d > 0 ? l.d : l.ua;  // closed_at если есть, иначе updated_at
  ...
  (function() {{
    ...
    // Аномалии > 30 дней (720 ч): игнорируем, если их < 5%
    ...
    // Подпись под заголовком графика
